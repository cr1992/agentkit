...
实时发现。配置使用 JSON，解析器基于 Node.js 18+，可在 Windows、macOS 与 Linux 运行。
...
`policy.json` 与 `hosts/<host>.json` 支持 `budget_mode`（`economy` / `balanced` / `quality`）与 `budget_modes` 覆盖块：

- **`economy` 模式**：优先下调 effort 档位（沿 `effort_order` 降 1 档，但受 `minimum_effort` 约束），收紧重试上限；
- **`balanced` 模式（默认）**：标准 profile 映射；
- **`quality` 模式**：优先上调 effort 档位（沿 `effort_order` 升 1 档），适用于关键生产与安全审计任务。

可在 `policy.json` 或 `hosts/<host>.json` 中显式定义 `budget_modes` 对特定模式的 routes 或 profiles 进行局部覆盖：
...
- 当轮 `--model / --effort / --budget-mode` 是用户显式覆盖，但仍受 allowed models 和 minimum effort 约束。
- 配置或实时 schema 无法满足严格要求时派发前阻塞；当前 schema 不提供隐式 fallback。

## 解析与验收

```text
node "<skill-directory>/scripts/resolve_model_policy.mjs" \
  --host host-a \
  --repo /path/to/repo \
  --role worker \
  --task-type flutter_implementation \
  --budget-mode balanced \
  --controller-model provider-primary-current \
...
profile、精确执行配置、来源文件、派发 provenance 与模型塌缩/倒置警告，原样进入任务契约与 Agent 台账。

当传入 `--controller-model` 时，解析器会自动检查：
- **模型塌缩（Model Collapse）**：若 Worker 解析出的模型与 Controller 相同，提示分发缺乏算力杠杆；
- **模型倒置（Model Inversion）**：若 Worker 解析出的模型档次高于 Controller，发出告警提示。
