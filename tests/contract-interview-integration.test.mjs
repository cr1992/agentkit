...
import { main as verifyMain, skillContentDigest as verifySkillDigest, RUNTIME_VERSION as VERIFY_RUNTIME_VERSION } from '../domains/verify/verification-runtime.mjs';
...
test('contract scaffold 与 verify scaffold --kind contract 输出同一份骨架', () => {
  ...
  try {
    ...
    for (const field of ['schema_version', 'objective', 'scope', 'acceptance', 'permissions', 'environment', 'stop_conditions', 'extensions']) {
    ...
    assert.deepEqual(fromContract.skill_set.map((item) => item.name), ['orchestrate-subagents']);
    assert.deepEqual(fromVerify.skill_set.map((item) => item.name), ['verify-agent-output']);
...
test('interview 冻结的契约配一份合规 profile 能通过 verify preflight', () => {
  ...
  try {
    ...
    draft.skill_set.push({ name: 'verify-agent-output', version: VERIFY_RUNTIME_VERSION, content_digest: verifySkillDigest(), provider_mode: 'primary' });
    ...
    const write = (name, value) => { const path = join(box.dir, name); writeFileSync(path, JSON.stringify(value)); return path; };
    ...
    profile.l0_checks = [{ check_id: 'readme-present', argv: ['node', '-e', 'require("node:fs").statSync("README.md")'], cwd_rel: '.', stage: 'both', timeout_ms: 30_000, expected_exit_codes: [0] }];
    profile.l1_review = [{ contract_item_id: frozen.acceptance[0].contract_item_id, lenses: ['functional', 'scope', 'verification_definition', 'safety'] }];
    ...
    const artifactPath = write('artifact.json', verifyMain(['scaffold', '--kind', 'artifact', '--workdir', repo, '--base-sha', head]));

    const result = verifyMain(['preflight', '--contract', contractPath, '--profile', profilePath, '--artifact', artifactPath]);
