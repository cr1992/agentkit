...
// 被测的是 orchestrate-subagents 的五个 CLI 入口；本文件随 cli-help 一起移出 Skill 目录后仍指向它们。
...
const CLIS = [
  [
    'contract-tool.mjs',
    ['normalize --input <value>', 'project --input <value> --items <value> [--contract-id <value>]'],
  ],
  [
    'orchestration-ledger.mjs',
    ['init --contract <value> [--state-root <value>]', 'attach --ledger <value> --node <value> --type <value>'],
  ],
...
test('未知命令仍返回各脚本原有的错误形状与非零退出，不被 --help 吞掉', () => {
  const run = (script) =>
    spawnSync(process.execPath, [join(SCRIPTS, script), 'definitely-not-a-command'], { encoding: 'utf8' });
  ...
  for (const [script, prefix] of [
    ['worker-capability-preflight.mjs', 'worker capability preflight error'],
    ['orchestration-reflection.mjs', 'orchestration reflection error'],
  ]) {
    const result = run(script);
    assert.equal(result.status, 2, `${script}: ${result.stdout}`);
...
test('帮助清单由 CLI_SPEC 表机械渲染，required / optional / flag 各自成形', () => {
  const help = renderCliHelp('demo.mjs', { only: {}, run: { required: ['a'], optional: ['b'], flags: ['c'] } }, [
    '说明行',
  ]);
...
test('无参数与三个求助 token 都算求助，真实命令不算', () => {
  ...
  for (const argv of [['status'], ['helper'], ['--helpful']])
    assert.equal(isHelpRequest(argv), false, JSON.stringify(argv));
});
