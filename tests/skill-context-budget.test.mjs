...
test('Skill 发现描述保持精简且可区分', () => {
  ...
  assert.ok(
    Object.values(descriptions).every((chars) => chars <= DESCRIPTION_LIMIT_PER_SKILL),
    `description 单项超限：${JSON.stringify(descriptions)}`,
  );
  assert.ok(
    Object.values(descriptions).reduce((sum, chars) => sum + chars, 0) <= DESCRIPTION_LIMIT_TOTAL,
    `description 总量超限：${JSON.stringify(descriptions)}`,
  );
});
