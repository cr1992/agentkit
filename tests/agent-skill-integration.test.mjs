...
import { main as loopMain, skillContentDigest as loopSkillDigest } from '../domains/loop/loop-runtime.mjs';
import { canonicalJson as worktreeCanonicalJson, worktreeSkillDigest } from '../domains/worktree/worktree-mgr.mjs';
import {
  main as ledgerMain,
  skillContentDigest as orchestratorSkillDigest,
    ...
    skill_set: [
      {
        ...
        version: '1.0.0',
        content_digest: verifierSkillDigest(),
      ...
      { name: 'run-agent-verify-loop', version: '1.0.0', content_digest: loopSkillDigest(), provider_mode: 'primary' },
      {
        ...
        version: '1.0.0',
        content_digest: orchestratorSkillDigest(),
        provider_mode: 'primary',
      },
    ],
    stop_conditions: [],
    ...
    l0_checks: [
      {
        ...
        argv: [
          'node',
          '-e',
          'const s=require("node:fs").readFileSync("result.txt","utf8");process.exit(s.includes("verified artifact")?0:1)',
        ],
        cwd_rel: '.',
        stage: 'both',
        timeout_ms: 10_000,
...
test('verifier 真实 Evidence 可被 Loop 消费并推进 orchestration 独立验收门禁', () => {
  ...
  try {
    ...
    const verification = verifierMain([
      'init',
      '--contract',
      contractPath,
      '--profile',
      ...
      value.repo,
      '--state-root',
      join(value.sandbox, 'verify-state'),
      '--isolation-assurance',
      'host_reported',
    ]);
    ...
    const completed = loopMain([
      'record-evidence',
      '--loop',
      loop.loop_dir,
      '--evidence',
      join(verification.run_dir, 'evidence.json'),
    ]);
    ...
    const ledger = ledgerMain([
      ...
      contractPath,
      '--state-root',
      join(value.sandbox, 'ledger-state'),
    ]);
    const nodePath = join(value.sandbox, 'verified-node.json');
    writeFileSync(
      nodePath,
      JSON.stringify({
        node_id: 'integration-candidate',
        objective: '验收最终集成候选',
        verification: {
...
test('manage-worktrees 产出的 Artifact Ref 可直接进入一次性 Verification', () => {
  ...
  try {
    ...
    git(repo, ['commit', '-m', 'chore: base']);
    const manager = (args) =>
      execFileSync(process.execPath, [WORKTREE_MANAGER, ...args], {
        cwd: repo,
        encoding: 'utf8',
        env: { ...process.env, WORKTREE_ROOT: join(sandbox, '.worktrees') },
      }).trim();
    manager([
      'spawn',
      'verified-artifact',
    ...
    git(workdir, ['commit', '-m', 'feat: artifact']);
    const artifact = JSON.parse(manager(['artifact', 'verified-artifact', '--json']));
    const contract = {
      schema_version: 1,
      contract_id: 'managed-artifact-contract',
      objective: '验证 worktree Artifact',
      scope: { include: ['result.txt'], exclude: [] },
      acceptance: [{ contract_item_id: 'content', requirement: 'result.txt 包含 verified artifact' }],
      permissions: { mode: 'read_only', writable_paths: [] },
      environment: { repository: workdir, isolation: 'worktree' },
      skill_set: [
        {
    ...
    const profile = {
      ...
      l0_checks: [
        {
          ...
          argv: [
            'node',
            '-e',
            'process.exit(require("node:fs").readFileSync("result.txt","utf8").includes("verified artifact")?0:1)',
          ],
          cwd_rel: '.',
          stage: 'both',
          timeout_ms: 10000,
      ...
      l1_review: [{ contract_item_id: 'content', lenses: ['functional'] }],
      protected_verifier_paths: [],
      allowed_validation_changes: [],
      runtime: {
        env_allowlist: ['PATH'],
    ...
    profile.verification_profile_digest = verifierDigest(profile, 'verification_profile_digest');
    for (const [name, value] of Object.entries({ contract, profile, artifact }))
      writeFileSync(join(sandbox, `${name}.json`), JSON.stringify(value));
    const run = verifierMain([
      'init',
      '--contract',
      join(sandbox, 'contract.json'),
      '--profile',
      join(sandbox, 'profile.json'),
      '--artifact',
      join(sandbox, 'artifact.json'),
      '--workdir',
      workdir,
      '--state-root',
      join(sandbox, 'verify'),
    ...
    const review = {
      schema_version: 1,
      review_result_id: 'managed-review',
      contract_digest: contract.contract_digest,
      verification_profile_digest: profile.verification_profile_digest,
      artifact_ref: artifact,
      challenge_nonce: run.review_challenge_nonce,
      verdict: 'no_defect_found',
      findings: [],
      forensics: ['读取冻结 worktree 中的 result.txt'],
    };
    review.review_result_digest = verifierDigest(review, 'review_result_digest');
  ...
  try {
    ...
    for (let index = 1; index <= 3; index += 1) {
      ...
      const loop = loopMain([
        ...
        'verify-agent-output',
        '--state-root',
        join(value.sandbox, 'batch-loop-state'),
        '--on-failure',
        'stop',
      ]);
      loopMain([
      ...
      const evidence = {
        ...
        artifact_ref: value.artifact,
        stages: {
          smoke_l0: { passed: false, checks: [{ check_id: 'content', passed: false }] },
          l1_review: { status: 'not_run' },
          final_l0: { status: 'not_run' },
        },
        ...
        provenance: {
          ...
          verifier_run_id: 'not_run',
          isolation_assurance: 'host_reported',
          limitations: ['l1_not_run'],
        },
      };
      evidence.evidence_digest = verifierDigest(evidence, 'evidence_digest');
      const evidencePath = join(value.sandbox, `batch-evidence-${index}.json`);
      writeFileSync(evidencePath, JSON.stringify(evidence));
      const stopped = loopMain(['record-evidence', '--loop', loop.loop_dir, '--evidence', evidencePath]);
      assert.equal(stopped.state, 'stopped');
      loopResults.push({
        loop_id: stopped.loop_id,
        failure_key: stopped.iterations[0].failure_signature,
        result_ref: join(loop.loop_dir, stopped.convergence_report_ref.ref),
      });
    }
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        loop_ids: loopResults.map((item) => item.loop_id),
    ...
    for (const [index, item] of loopResults.entries()) {
      const path = join(value.sandbox, `batch-record-${index}.json`);
      writeFileSync(path, JSON.stringify({ ...item, state: 'stopped' }));
