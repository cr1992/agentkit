...
function initialize(fixture) {
  const initialized = main([
    'init',
    '--contract',
    join(fixture.sandbox, 'contract.json'),
    '--profile',
    join(fixture.sandbox, 'profile.json'),
    '--artifact',
    join(fixture.sandbox, 'artifact.json'),
    '--workdir',
    fixture.repo,
    '--state-root',
    fixture.stateRoot,
...
test('prepare-run 只读规范化输入并一次完成 readiness、preflight 与 init', () => {
  ...
  try {
    ...
    const cli = spawnSync(
      process.execPath,
      [
        join(SCRIPT_DIR, 'verification-runtime.mjs'),
        'prepare-run',
        '--contract',
        contractPath,
        '--profile',
    ...
    assert.equal(
      frozenProfile.verification_profile_digest,
      envelopeDigest(frozenProfile, 'verification_profile_digest'),
    );
  } finally {
    fixture.cleanup();
  }
...
test('CLI 子命令支持 --help，状态变更默认 compact 且 --verbose 保留完整 snapshot', () => {
  ...
  try {
    ...
    const compactCli = spawnSync(process.execPath, [script, 'run-smoke', '--run', initialized.run_dir], {
      encoding: 'utf8',
    });
    ...
    try {
      ...
      const verboseCli = spawnSync(process.execPath, [script, 'run-smoke', '--run', next.run_dir, '--verbose'], {
        encoding: 'utf8',
      });
...
test('preflight 与 init 一次汇总 Profile 形状、枚举、摘要和隔离 assurance 错误', () => {
  ...
  try {
    ...
    const cli = spawnSync(process.execPath, [join(SCRIPT_DIR, 'verification-runtime.mjs'), 'preflight', ...args], {
      encoding: 'utf8',
    });
    ...
    assert.throws(
      ...
      /cache_policy[\s\S]*human_gate|human_gate[\s\S]*cache_policy/,
    );
  } finally {
    fixture.cleanup();
  }
  ...
  try {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        observation: `Bearer hidden-token at /Users/example/project`,
        evidence_refs: [{ type: 'evidence', id: 'evidence.json', digest: evidenceDigest }],
        impact: 'medium',
        confidence: 'high',
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        proposed_change: '增加一个可回放的边界检查',
        affected_scope: ['verification profile'],
        counterexamples: [],
        validation_plan: { replay_cases: ['case-1'], regression_suites: ['runtime'], independent_review: 'required' },
        lifecycle: 'proposed',
      }),
    );
    const proposed = main([
    ...
    const proposal = JSON.parse(
      readFileSync(join(initialized.run_dir, proposed.improvement_proposal_refs[0].ref), 'utf8'),
    );
...
test('反思面拒绝无证据高置信输入，且不改变验收终态', () => {
  ...
  try {
    ...
    assert.throws(
      () => main(['record-reflection', '--run', initialized.run_dir, '--input', bad]),
      /只能是 low confidence/,
    );
...
test('CLI typo、Review replay 与伪造 journal 都 fail closed', () => {
  ...
  try {
    ...
    const forged = {
      ...
      snapshot: { ...snapshot, revision: snapshot.revision + 1, status: 'terminal' },
    };
...
test('空 Profile、非法 H gate 与非字符串 finding 不能进入执行链', () => {
  ...
  try {
    const initialized = initialize(fixture);
    main(['run-smoke', '--run', initialized.run_dir]);
    const review = validReview(fixture, 'fail');
    review.findings[0].evidence = [];
    review.review_result_digest = envelopeDigest(review, 'review_result_digest');
    const path = join(fixture.sandbox, 'typed-review.json');
...
test('capabilities --json 保持统一能力发现兼容', () => {
  const output = execFileSync(
    process.execPath,
    [join(SCRIPT_DIR, 'verification-runtime.mjs'), 'capabilities', '--json'],
    { encoding: 'utf8' },
  );
  const value = JSON.parse(output);
  assert.equal(value.skill, 'verify-agent-output');
  assert.equal(value.runtime_version, RUNTIME_VERSION);
  for (const feature of ['review-bundle', 'readiness-preconditions', 'prepare-scaffold-chain'])
    assert.ok(value.features.includes(feature), feature);
});
...
test('review-bundle 打包自包含派发输入，并按投影标注 contract_kind', () => {
  for (const item of [
    { kind: 'public', extensions: {} },
    {
      kind: 'projected',
      extensions: {
        projection: { parent_contract_digest: `sha256:${'a'.repeat(64)}`, projected_item_ids: ['content'] },
      },
    },
  ]) {
    ...
    try {
      const initialized = initialize(fixture);
      assert.throws(
        () => main(['review-bundle', '--run', initialized.run_dir]),
        /review-input 不接受状态 initialized/u,
      );
      main(['run-smoke', '--run', initialized.run_dir]);
      ...
      assert.equal(
        canonicalJson(bundle.review_result_schema),
        canonicalJson(
          parseJsonStrict(
            readFileSync(join(SCRIPT_DIR, '..', '..', 'schemas', 'review-result-v1.schema.json'), 'utf8'),
          ),
        ),
      );
      ...
      const falsification = protocol
        .split(/^## /mu)
        .find((part) => part.startsWith('证伪任务'))
        .match(/```text\n([\s\S]*?)\n```/u)[1];
      assert.ok(bundle.reviewer_prompt.includes(falsification), '提示词必须内联协议真源的证伪任务原文');
      for (const token of [
        'fail | no_defect_found | undecidable',
        'contract_item_id / class / evidence / expected / actual',
        'forensics 必须是非空字符串数组',
        'safety finding',
        'blocked_safety',
        '只读审查',
      ]) {
...
test('readiness 只判环境前提：正例 ready，缺可执行/坏 workdir/state-root 不可写都是 precondition blocker', () => {
  ...
  try {
    ...
    assert.equal(
      ready.blockers.some((item) => item.check_id.startsWith('env')),
      false,
    );
    ...
    assert.ok(
      badState.blockers.some((item) => item.check_id === 'state_root_writable' && item.kind === 'precondition'),
    );
    if (process.getuid?.() !== 0) {
      ...
      try {
        const denied = main([
          ...
          fixture.repo,
          '--state-root',
          join(locked, 'runs-root'),
        ]);
        assert.equal(denied.ready, false);
        assert.ok(
          denied.blockers.some((item) => item.check_id === 'state_root_writable' && /不可写/u.test(item.detail)),
        );
      } finally {
    ...
    const unreadable = main([
      'readiness',
      '--contract',
      join(fixture.sandbox, 'absent.json'),
      '--profile',
      profilePath,
      '--workdir',
      fixture.repo,
...
test('prepare 只出骨架与 TODO，不猜测试命令；契约 TODO 与 L0 都填实后才过 preflight', () => {
  ...
  try {
    ...
    assert.ok(
      prepared.next_steps.some((item) => item.includes('readiness')) &&
        prepared.next_steps.some((item) => item.includes('preflight')),
    );
    const serialized = JSON.stringify(prepared);
    for (const guessed of ['flutter', 'pytest', 'npm test', 'just ', 'cargo', 'gradle'])
      assert.equal(serialized.toLowerCase().includes(guessed), false, guessed);
    ...
    assert.deepEqual(
      partial.errors.map((item) => item.split(' = ')[0]),
      ['objective', 'acceptance[0].requirement', 'scope.include[0]'],
    );
    ...
    assert.equal(
      inline.profile.verification_profile_digest,
      envelopeDigest(inline.profile, 'verification_profile_digest'),
    );
  } finally {
    fixture.cleanup();
  }
