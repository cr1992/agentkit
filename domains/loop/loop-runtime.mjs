...
export function skillContentDigest(root = SKILL_ROOT) {
  return distributionDigest(
    skillDistributionRoots({
      ...
      docsRoot: join(PACKAGE_ROOT, 'docs', 'loop'),
    }),
  );
}
...
function recoverOrphanReclaim(path) {
  ...
  try {
    ...
    throw new LoopValidationError('state-root lock reclaim 内容损坏；拒绝自动接管');
  }
  if (!Number.isInteger(Number(owner.pid)) || Number(owner.pid) <= 0 || typeof owner.token !== 'string' || !owner.token)
    throw new LoopValidationError('state-root lock reclaim owner 无效；拒绝自动接管');
  ...
  for (let attempt = 0; attempt < 4; attempt += 1) {
    ...
    try {
      ...
      try {
        ...
        throw new LoopValidationError('state-root lock 内容损坏；拒绝自动接管');
      }
      if (!Number.isInteger(Number(current.pid)) || Number(current.pid) <= 0)
        throw new LoopValidationError('state-root lock owner 无效；拒绝自动接管');
      if (processIsAlive(Number(current.pid)))
        throw new LoopValidationError(`state-root lock 正被 PID ${current.pid} 持有`);
      const reclaimPath = `${path}.reclaim`;
      ...
      try {
        ...
        throw new LoopValidationError('state-root lock 正在执行 stale recovery');
      } finally {
        try {
      ...
      try {
        ...
        if (processIsAlive(Number(latest.pid)))
          throw new LoopValidationError(`state-root lock 正被 PID ${latest.pid} 持有`);
        unlinkSync(path);
      } finally {
...
export function releaseLock(path, owner) {
  ...
  try {
    ...
    throw new LoopValidationError('state-root lock 在持有期间损坏；拒绝删除未知 owner 的 lock');
  }
...
function validateArtifact(artifact) {
  ...
  for (const field of ['provider', 'repository_id', 'object_format', 'base_sha', 'artifact_sha'])
    if (!artifact[field]) throw new LoopValidationError(`Artifact 缺少 ${field}`);
  if (!['manage-worktrees', 'caller-supplied'].includes(artifact.provider))
    throw new LoopValidationError('Artifact provider 非法');
  const size = artifact.object_format === 'sha1' ? 40 : artifact.object_format === 'sha256' ? 64 : 0;
  if (
    !size ||
    !new RegExp(`^[0-9a-f]{${size}}$`, 'u').test(artifact.base_sha) ||
    !new RegExp(`^[0-9a-f]{${size}}$`, 'u').test(artifact.artifact_sha)
  )
    throw new LoopValidationError('Artifact object format 或 SHA 非法');
}
...
function validateProfilePath(path) {
  if (
    ...
    throw new LoopValidationError(`非法 verifier path: ${path}`);
  const core = path.endsWith('/') ? path.slice(0, -1) : path;
  if (!core || core.split('/').some((part) => !part || part === '.' || part === '..'))
    throw new LoopValidationError(`非法 verifier path: ${path}`);
}

function profilePathMatches(path, rule) {
...
function verifyProtectedPaths(artifact, profile, workdir) {
  ...
  const bytes = git(
    ['diff', '--name-status', '-z', '--no-renames', artifact.base_sha, artifact.artifact_sha],
    workdir,
    'buffer',
  );
  ...
  const violations = changed.filter(
    (path) =>
      protectedPaths.some((rule) => profilePathMatches(path, rule)) &&
      !allowed.some((rule) => profilePathMatches(path, rule)),
  );
  if (violations.length > 0)
    throw new LoopOperationalAbort('protected_path_violation', `未授权修改 verifier path: ${violations.join(', ')}`);
}
...
function verifyGitArtifact(artifact, workdir, frozenIdentity = null) {
  ...
  if (root !== realpathSync(workdir))
    throw new LoopOperationalAbort('stale_precondition', 'workdir 必须是 worktree 根目录');
  const objectFormat = String(git(['rev-parse', '--show-object-format'], root)).trim();
  if (objectFormat !== artifact.object_format)
    throw new LoopOperationalAbort('stale_precondition', 'object format 已变化');
  for (const field of ['base_sha', 'artifact_sha'])
    if (String(git(['cat-file', '-t', artifact[field]], root)).trim() !== 'commit')
      throw new LoopValidationError(`${field} 不是 commit`);
  if (gitStatus(['merge-base', '--is-ancestor', artifact.base_sha, artifact.artifact_sha], root).status !== 0)
    throw new LoopValidationError('base_sha 不是 artifact ancestor');
  if (String(git(['rev-parse', 'HEAD'], root)).trim() !== artifact.artifact_sha)
    throw new LoopOperationalAbort('stale_precondition', 'HEAD 偏离 Artifact');
  if (git(['status', '--porcelain=v1', '-z', '--untracked-files=all'], root, 'buffer').length > 0)
    throw new LoopOperationalAbort('stale_precondition', 'embedded workdir 不干净');
  ...
  if (frozenIdentity && frozenIdentity !== identity)
    throw new LoopOperationalAbort('stale_precondition', 'repository identity 已变化');
  return identity;
}

function validateContract(contract) {
  if (contract?.schema_version !== 1 || !contract.contract_id || !contract.objective)
    throw new LoopValidationError('Task Contract schema/id/objective 无效');
  if (
    !contract.scope ||
    ...
    throw new LoopValidationError('Task Contract scope 无效');
  if (
    ...
    (contract.permissions.mode === 'read_only' && contract.permissions.writable_paths.length)
  )
    throw new LoopValidationError('Task Contract permissions 无效');
  if (
    ...
    throw new LoopValidationError('Task Contract environment 无效');
  if (
    ...
    throw new LoopValidationError('Task Contract stop_conditions/extensions 无效');
  if (!Array.isArray(contract.acceptance) || contract.acceptance.length === 0)
    throw new LoopValidationError('acceptance 不能为空');
  const ids = new Set();
  for (const item of contract.acceptance) {
    if (!item?.contract_item_id || !item?.requirement || ids.has(item.contract_item_id))
      throw new LoopValidationError('acceptance ID 缺失或重复');
    ids.add(item.contract_item_id);
  }
  if (!Array.isArray(contract.skill_set)) throw new LoopValidationError('skill_set 必须是数组');
  if (
    !DIGEST_PATTERN.test(contract.contract_digest) ||
    envelopeDigest(contract, 'contract_digest') !== contract.contract_digest
  )
    throw new LoopValidationError('Task Contract digest 无效');
  return ids;
}

function validateProfile(profile, ids) {
  try {
    validateJsonSchema(profile, schema('verification-profile-v1.schema.json'), 'Verification Profile');
  } catch (error) {
    throw new LoopValidationError(error instanceof Error ? error.message : String(error));
  }
  if (
    ...
    !Array.isArray(profile.l1_review) ||
    profile.l1_review.length === 0
  )
    throw new LoopValidationError('Verification Profile 无效');
  const checks = new Set();
  for (const check of profile.l0_checks) {
    if (
      !check?.check_id ||
      checks.has(check.check_id) ||
      !Array.isArray(check.argv) ||
      check.argv.length === 0 ||
      check.argv.some((item) => typeof item !== 'string')
    )
      throw new LoopValidationError('L0 check 无效或重复');
    checks.add(check.check_id);
    if (
      !['smoke', 'final', 'both'].includes(check.stage) ||
      !Number.isInteger(check.timeout_ms) ||
      check.timeout_ms <= 0 ||
      !Array.isArray(check.expected_exit_codes) ||
      check.expected_exit_codes.length === 0 ||
      check.expected_exit_codes.some((code) => !Number.isInteger(code))
    )
      throw new LoopValidationError(`${check.check_id} 配置无效`);
    if (
      !check.cwd_rel ||
      ...
      throw new LoopValidationError(`${check.check_id} cwd_rel 非法`);
  }
  if (
    !profile.l0_checks.some((check) => ['smoke', 'both'].includes(check.stage)) ||
    !profile.l0_checks.some((check) => ['final', 'both'].includes(check.stage))
  )
    throw new LoopValidationError('Verification Profile 必须覆盖 smoke 与 final L0');
  for (const review of profile.l1_review)
    if (!ids.has(review?.contract_item_id))
      throw new LoopValidationError(`L1 引用未知 acceptance: ${review?.contract_item_id}`);
  if (!Array.isArray(profile.protected_verifier_paths) || !Array.isArray(profile.allowed_validation_changes))
    throw new LoopValidationError('Profile path 字段必须是数组');
  for (const path of [...profile.protected_verifier_paths, ...profile.allowed_validation_changes])
  ...
  if (
    ...
    !Number.isInteger(profile.runtime.max_log_bytes) ||
    profile.runtime.max_log_bytes <= 0
  )
    throw new LoopValidationError('Profile runtime 无效');
  if (
    !['disabled', 'isolated', 'trusted_identity'].includes(profile.runtime.cache_policy) ||
    !['denied', 'contract_authorized'].includes(profile.runtime.network_policy)
  )
    throw new LoopValidationError('Profile cache/network policy 无效');
  if (!['none', 'release', 'destructive', 'external_side_effect'].includes(profile.human_gate))
    throw new LoopValidationError('human_gate 非法');
  for (const check of profile.l0_checks) {
    ...
    if (!executable || !isAbsolute(executable) || !statSync(executable).isFile())
      throw new LoopValidationError(`未冻结 executable: ${check.argv[0]}`);
  }
  if (
    !DIGEST_PATTERN.test(profile.verification_profile_digest) ||
    envelopeDigest(profile, 'verification_profile_digest') !== profile.verification_profile_digest
  )
    throw new LoopValidationError('Profile digest 无效');
}
...
function validateSkillBinding(contract, contentDigest) {
  ...
  if (!entry || entry.content_digest !== contentDigest)
    throw new LoopValidationError('Task Contract 未绑定当前 run-agent-verify-loop 内容摘要');
}
...
function validateReview(review, snapshot, ids) {
  ...
  if (
    ...
    !['fail', 'no_defect_found', 'undecidable'].includes(review.verdict)
  )
    throw new LoopValidationError('Review Result schema/id/verdict 无效');
  if (
    review.contract_digest !== snapshot.contract_digest ||
    review.verification_profile_digest !== snapshot.verification_profile_digest ||
    canonicalJson(review.artifact_ref) !== canonicalJson(snapshot.current_iteration.artifact_ref)
  )
    throw new LoopValidationError('Review Result binding 不匹配');
  if (review.challenge_nonce !== snapshot.current_iteration.review_challenge_nonce)
    throw new LoopValidationError('Review Result challenge nonce 不匹配或已重放');
  if (!Array.isArray(review.findings) || !Array.isArray(review.forensics))
    throw new LoopValidationError('Review Result findings/forensics 无效');
  for (const finding of review.findings) {
    if (!ids.has(finding?.contract_item_id) || !FINDING_CLASSES.has(finding?.class))
      throw new LoopValidationError('finding ID/class 无效');
    for (const field of ['evidence', 'expected', 'actual'])
      if (typeof finding[field] !== 'string' || !finding[field].trim())
        throw new LoopValidationError(`finding ${field} 必须是非空字符串`);
  }
  if (review.forensics.some((item) => typeof item !== 'string' || !item.trim()))
    throw new LoopValidationError('forensics 必须是非空字符串数组');
  ...
  if (review.verdict === 'no_defect_found' && (review.findings.length > 0 || review.forensics.length === 0))
    throw new LoopValidationError('no_defect_found 必须无 finding 且有 forensics');
  if (review.verdict === 'undecidable' && review.findings.length === 0)
    throw new LoopValidationError('undecidable 必须说明证据缺口');
  if (
    !DIGEST_PATTERN.test(review.review_result_digest) ||
    envelopeDigest(review, 'review_result_digest') !== review.review_result_digest
  )
    throw new LoopValidationError('Review Result digest 无效');
}
...
function validateEvidence(evidence, snapshot) {
  if (
    ...
    evidence.provenance?.provider !== 'verify-agent-output'
  )
    throw new LoopValidationError('Evidence schema/protocol/provider 无效');
  if (
    typeof evidence.runtime_version !== 'string' ||
    !/^\d+\.\d+\.\d+(?:[-+][0-9A-Za-z.-]+)?$/u.test(evidence.runtime_version)
  )
    throw new LoopValidationError('Evidence runtime_version 非法');
  if (
    !['pass', 'fail', 'undecidable', 'blocked_safety'].includes(evidence.terminal_outcome) ||
    evidence.completion_scope !== 'verification_only'
  )
    throw new LoopValidationError('Evidence outcome/scope 无效');
  if (
    ...
    canonicalJson(evidence.artifact_ref) !== canonicalJson(snapshot.current_iteration.artifact_ref)
  )
    throw new LoopValidationError('Evidence binding 不匹配');
  if (evidence.run_id !== snapshot.current_iteration.verification_run_id)
    throw new LoopValidationError('Evidence run_id 与 iteration 不一致');
  if (evidence.terminal_outcome === 'fail') {
    ...
    if (!checks.some((item) => item?.passed === false) && findings.length === 0)
      throw new LoopValidationError('fail Evidence 缺少稳定失败依据');
  }
  if (
    !DIGEST_PATTERN.test(evidence.evidence_digest) ||
    envelopeDigest(evidence, 'evidence_digest') !== evidence.evidence_digest
  )
    throw new LoopValidationError('Evidence digest 无效');
}
...
function executeEmbeddedChecks(snapshot, loopDir) {
  ...
  for (const check of profile.l0_checks) {
    ...
    try {
      ...
      const stat = statSync(absolute);
      live = { path: absolute, size: stat.size, mode: stat.mode, digest: sha256(readFileSync(absolute)) };
    } catch (error) {
      throw new LoopOperationalAbort(
        'check_runtime_failure',
        `${check.check_id} executable 无法执行: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
    if (
      !snapshot.executable_identities?.[check.argv[0]] ||
      canonicalJson(live) !== canonicalJson(snapshot.executable_identities[check.argv[0]])
    )
      throw new LoopOperationalAbort('check_runtime_failure', `冻结 executable 已变化: ${check.argv[0]}`);
    for (const [key, identity] of Object.entries(snapshot.argv_file_identities ?? {})) {
      ...
      let argumentLive;
      try {
        argumentLive = { argument_path: resolve(candidate), ...fileIdentity(candidate) };
      } catch (error) {
        throw new LoopOperationalAbort(
          'check_runtime_failure',
      ...
      if (canonicalJson(argumentLive) !== canonicalJson(identity))
        throw new LoopOperationalAbort('check_runtime_failure', `冻结 argv 文件已变化: ${key}`);
    }
    ...
    if (exitCode === null && !timedOut)
      throw new LoopOperationalAbort(
        'check_runtime_failure',
        `${check.check_id} 未产生退出码，诊断 ${persisted.log_ref}`,
      );
    results.push({
...
function readJournal(loopDir) {
  ...
  for (let index = 0; index < events.length; index += 1) {
    ...
    if (
      ...
      throw new LoopValidationError(`event journal 链在 revision ${index} 无效`);
...
function ensureConvergenceReport(loopDir, snapshot) {
  ...
  const report = {
    ...
    strategy_changes: snapshot.iterations
      .filter((item) => item.next_action)
      .map((item) => ({ iteration: item.index, action: item.next_action })),
    verification_aborts: snapshot.current_iteration?.verification_abort
      ? [snapshot.current_iteration.verification_abort]
      : snapshot.terminal?.kind === 'verification_abort'
        ? [snapshot.terminal]
        : [],
    verification_gaps: snapshot.iterations
      .filter((item) => item.outcome === 'undecidable')
      .map((item) => ({ iteration: item.index, outcome: item.outcome })),
  ...
  if (existsSync(join(loopDir, ref))) {
    ...
    if (envelopeDigest(existing, 'report_digest') !== existing.report_digest || existing.loop_id !== snapshot.loop_id)
      throw new LoopValidationError('已有 Convergence Report 无效');
    return {
      ...snapshot,
      convergence_report_ref: { ref, report_id: existing.report_id, report_digest: existing.report_digest },
    };
  }
  writeNewJson(join(loopDir, ref), report);
  return {
    ...snapshot,
    convergence_report_ref: { ref, report_id: report.report_id, report_digest: report.report_digest },
  };
}
...
function stateRootForLoop(loopDir) {
  ...
  if (
    ...
    identity.canonical_path !== stateRoot ||
    !DIGEST_PATTERN.test(identity.identity_digest ?? '') ||
    envelopeDigest(identity, 'identity_digest') !== identity.identity_digest
  )
    throw new LoopValidationError('state root identity 与当前位置不匹配，拒绝复制态重放');
  return stateRoot;
}
...
function initializeStateRootIdentity(stateRoot) {
  ...
  if (existsSync(path)) {
    ...
    if (
      ...
      throw new LoopValidationError('state root identity 无效');
    return;
  }
...
function adoptStateRoot(options) {
  ...
  if (!existsSync(loopsDir) || !lstatSync(loopsDir).isDirectory())
    throw new LoopValidationError('adopt-root 只接受已有 loops/ 的 state root');
  return withRootLock(stateRoot, () => {
    ...
    for (const name of loopNames) {
      ...
      if (!lstatSync(loopDir).isDirectory() || lstatSync(loopDir).isSymbolicLink())
        throw new LoopValidationError(`非法 Loop 目录: ${name}`);
      ...
      if (
        loaded.snapshot.loop_id !== name ||
        loaded.snapshot.contract_digest !== contract.contract_digest ||
        loaded.snapshot.verification_profile_digest !== profile.verification_profile_digest
      )
        throw new LoopValidationError(`Loop ${name} 的冻结 envelope 与 journal 不一致`);
      const repository = contract.environment?.repository;
      if (
        typeof repository === 'string' &&
        repository !== 'none' &&
        existsSync(repository) &&
        pathInside(stateRoot, realpathSync(repository))
      )
        throw new LoopValidationError(`state root 位于 Loop ${name} 的业务仓库内，拒绝接管`);
    }
    const identityPath = join(stateRoot, '.state-root-identity.json');
    let previous = null;
    if (existsSync(identityPath)) {
      try {
        ...
        previous = {
          state_root_id: value.state_root_id ?? null,
          canonical_path: value.canonical_path ?? null,
          identity_digest: value.identity_digest ?? null,
        };
      } catch {
        previous = { state_root_id: null, canonical_path: null, identity_digest: null };
      }
    }
    const identity = {
      schema_version: 1,
      state_root_id: randomUUID(),
    ...
    return {
      ...
      state_root_id: identity.state_root_id,
...
function mutateLoop(loopDir, expectedRevision, callback) {
  ...
  return withRootLock(stateRoot, () => {
    ...
    if (expectedRevision !== null && snapshot.revision !== expectedRevision)
      throw new LoopValidationError(`revision 冲突：expected ${expectedRevision}, actual ${snapshot.revision}`);
      ...
      if (!['completed', 'stopped'].includes(snapshot.state)) {
        snapshot = persistTransition(
          loopDir,
          {
            ...snapshot,
            state: 'stopped',
            terminal: { kind: 'operational_abort', code: 'skill_drift', diagnostics: 'Skill 内容摘要发生变化' },
          },
          'skill_drift',
        );
    ...
    try {
      ...
      if (error instanceof LoopOperationalAbort && !['completed', 'stopped'].includes(snapshot.state))
        persistTransition(
          loopDir,
          {
            ...snapshot,
            state: 'stopped',
            terminal: { kind: 'operational_abort', code: error.code, diagnostics: error.message },
          },
          'operational_abort',
        );
      throw error;
...
export function failureSignature(l0Result, reviewResult) {
  ...
  for (const finding of reviewResult?.findings ?? [])
    keys.push({ kind: 'l1', contract_item_id: finding.contract_item_id, class: finding.class });
...
function applyOutcome(snapshot, outcome, source) {
  ...
  if (outcome === 'fail') {
    const inputs =
      source.kind === 'evidence'
        ? evidenceFailureInputs(source.value)
        : { l0: source.value.l0_result, review: source.value.l1_result };
  ...
  if (outcome === 'pass') {
    if (snapshot.human_gate.required)
      return {
        ...snapshot,
        iterations,
        current_iteration: null,
        state: 'waiting_human',
        human_gate: { ...snapshot.human_gate, status: 'pending' },
        terminal: null,
      };
    return {
      ...snapshot,
      ...
      current_iteration: null,
      state: 'completed',
      terminal: { kind: 'completed', outcome: 'pass' },
    };
  }
  if (outcome === 'blocked_safety' || outcome === 'undecidable')
    return { ...snapshot, iterations, current_iteration: null, state: 'stopped', terminal: { kind: outcome, outcome } };
  const consecutive = signature
    ? [...iterations].reverse().findIndex((item) => item.failure_signature !== signature)
    : 0;
  const count = consecutive === -1 ? iterations.length : consecutive;
  if (index >= snapshot.limits.max_iterations)
    return {
      ...snapshot,
      iterations,
      current_iteration: null,
      state: 'stopped',
      terminal: { kind: 'max_iterations', failure_signature: signature },
    };
  if (signature && count >= snapshot.limits.consecutive_identical_signature)
    return {
      ...snapshot,
      iterations,
      current_iteration: null,
      state: 'stopped',
      terminal: { kind: 'identical_failure_fuse', failure_signature: signature, consecutive: count },
    };
  if (snapshot.policy.on_failure === 'stop')
    return {
      ...snapshot,
      iterations,
      current_iteration: null,
      state: 'stopped',
      terminal: { kind: 'policy_stop', failure_signature: signature },
    };
  return {
    ...snapshot,
    iterations,
    ...
    state: 'active',
    terminal: null,
    next_action: snapshot.policy.on_failure,
  };
}
...
const CLI_SPEC = {
  ...
  init: {
    ...
    flags: ['allow-repository-state', 'network-isolated'],
  },
  ...
  'record-embedded-review': {
    values: ['loop', 'review', 'assurance', 'reviewer-run-id', 'expected-revision'],
    flags: [],
  },
  'record-evidence': { values: ['loop', 'evidence', 'expected-revision'], flags: [] },
  'record-verification-abort': {
    values: ['loop', 'run-id', 'code', 'diagnostics-ref', 'expected-revision'],
    flags: [],
  },
  next: { values: ['loop', 'expected-revision'], flags: [] },
  resume: { values: ['loop', 'verification-run-id', 'expected-revision'], flags: [] },
  stop: { values: ['loop', 'reason', 'expected-revision'], flags: [] },
  'human-gate': { values: ['loop', 'decision', 'expected-revision'], flags: [] },
  'record-reflection': { values: ['loop', 'input', 'expected-revision'], flags: [] },
  'convergence-report': { values: ['loop'], flags: [] },
  'propose-improvement': { values: ['loop', 'reflection', 'input', 'expected-revision'], flags: [] },
  status: { values: ['loop'], flags: [] },
  inspect: { values: ['loop'], flags: [] },
  validate: { values: ['loop'], flags: [] },
  doctor: { values: ['loop'], flags: [] },
};
  ...
  for (let index = 1; index < argv.length; index += 1) {
    ...
    if (spec.flags.includes(name)) {
      if (argv[index + 1] !== undefined && !argv[index + 1].startsWith('--'))
        throw new LoopValidationError(`--${name} 不接受值`);
      flags.add(name);
    } else if (spec.values.includes(name)) {
      if (argv[index + 1] === undefined || argv[index + 1].startsWith('--'))
        throw new LoopValidationError(`--${name} 缺少值`);
...
function required(options, name) {
  if (!options[name]) throw new LoopValidationError(`缺少 --${name}`);
  return options[name];
}
...
function capabilities() {
  return {
    ...
    contracts: {
      ...
      review_result: [1],
      evidence_package: [1],
      embedded_verification_record: [1],
      loop_state: [1],
      reflection_record: [1],
      convergence_report: [1],
    ...
    features: [
      ...
      'revision-lock',
      'failure-fuse',
      'state-root-unmodified-copy-detection',
      'state-root-explicit-adoption',
      'skill-drift',
      'human-gate',
...
function initialize(options, flags) {
  ...
  const substance = [
    ...
    ...profileSubstance(profile).errors,
    ...coverageSubstance(contract, profile).errors,
  ];
  ...
  const contractRepository =
    typeof contract.environment?.repository === 'string' &&
    contract.environment.repository !== 'none' &&
    existsSync(contract.environment.repository)
      ? realpathSync(contract.environment.repository)
      : null;
  if (
    ((workdir && pathInside(stateRoot, workdir)) ||
      (contractRepository && pathInside(stateRoot, contractRepository))) &&
    !flags.has('allow-repository-state')
  )
    throw new LoopValidationError('state root 位于仓库内；需显式授权');
  if (provider === 'embedded' && profile.runtime.network_policy === 'denied' && !flags.has('network-isolated'))
    throw new LoopValidationError('embedded network_policy=denied 时必须提供 --network-isolated assurance');
  ...
  if (!['retry', 'rollback', 'change_strategy', 'stop'].includes(onFailure))
    throw new LoopValidationError('on-failure 非法');
  return withRootLock(stateRoot, () => {
    ...
    const snapshot = {
      ...
      executable_identities: Object.fromEntries(
        Object.entries(profile.runtime.executable_paths).map(([name, path]) => [name, fileIdentity(path)]),
      ),
      ...
      human_gate: {
        required: profile.human_gate !== 'none',
        status: profile.human_gate === 'none' ? 'not_required' : 'pending',
      },
    ...
    return {
      ...
      state: 'active',
      provider,
      ...(contractReport.warnings.length ? { warnings: contractReport.warnings } : {}),
    };
  });
}
  ...
  return mutateLoop(loopDir, expectedRevision(options), (snapshot) => {
    const record = buildReflection({
      ...
      scope: {
        contract_digest: snapshot.contract_digest,
        loop_id: snapshot.loop_id,
        iteration: snapshot.current_iteration?.index ?? snapshot.iterations.at(-1)?.index ?? null,
      },
...
function convergenceReport(options) {
  ...
  if (!['completed', 'stopped'].includes(snapshot.state) || !snapshot.convergence_report_ref)
    throw new LoopValidationError('只有 completed/stopped Loop 才有 Convergence Report');
  const report = readJson(join(loopDir, snapshot.convergence_report_ref.ref));
  if (
    envelopeDigest(report, 'report_digest') !== report.report_digest ||
    report.report_digest !== snapshot.convergence_report_ref.report_digest
  )
    throw new LoopValidationError('Convergence Report digest 无效');
  return report;
}
...
function proposeImprovement(options) {
  ...
  return mutateLoop(loopDir, expectedRevision(options), (snapshot) => {
    ...
    if (!snapshot.reflection_refs.some((item) => item.reflection_digest === reflection.reflection_digest))
      throw new LoopValidationError('Reflection 未登记到当前 Loop');
...
function recordArtifact(options) {
  ...
  validateArtifact(artifact);
  return mutateLoop(loopDir, expectedRevision(options), (snapshot) => {
    if (snapshot.state !== 'active' || snapshot.current_iteration)
      throw new LoopValidationError('当前 Loop 不能登记新 Artifact');
    ...
    const verificationRunId =
      snapshot.provider === 'verify-agent-output' ? required(options, 'verification-run-id') : null;
    ...
    return {
      snapshot: {
        ...snapshot,
        runtime_repository_identity: identity,
        current_iteration: {
          index,
...
function runEmbeddedL0(options) {
  ...
  return mutateLoop(loopDir, expectedRevision(options), (snapshot) => {
    if (
      snapshot.provider !== 'embedded' ||
      snapshot.state !== 'active' ||
      !snapshot.current_iteration ||
      snapshot.current_iteration.embedded_l0
    )
      throw new LoopValidationError('当前状态不能运行 embedded L0');
    const result = executeEmbeddedChecks(snapshot, loopDir);
    return {
      snapshot: { ...snapshot, current_iteration: { ...snapshot.current_iteration, embedded_l0: result } },
      kind: 'embedded_l0_recorded',
    };
  });
}
...
function recordEmbeddedReview(options) {
  ...
  return mutateLoop(loopDir, expectedRevision(options), (snapshot) => {
    if (snapshot.provider !== 'embedded' || snapshot.state !== 'active' || !snapshot.current_iteration?.embedded_l0)
      throw new LoopValidationError('当前状态不能登记 embedded review');
    if (reviewerRunId === snapshot.loop_id || reviewerRunId === snapshot.current_iteration.verification_run_id)
      throw new LoopValidationError('reviewer-run-id 必须与 controller/verification run 隔离');
    const contract = readJson(join(loopDir, 'contract.json'));
    const ids = validateContract(contract);
    validateReview(review, snapshot, ids);
    const safety = review.findings.some((finding) => finding.class === 'safety');
    const outcome = safety
      ? 'blocked_safety'
      : review.verdict === 'undecidable'
        ? 'undecidable'
        : !snapshot.current_iteration.embedded_l0.passed || review.verdict === 'fail'
          ? 'fail'
          : 'pass';
    const record = {
      schema_version: 1,
    ...
    if (existsSync(path)) {
      ...
      if (
        envelopeDigest(persisted, 'record_digest') !== persisted.record_digest ||
    ...
    return {
      snapshot: applyOutcome(snapshot, persisted.outcome, { kind: 'embedded', value: persisted }),
      kind: `embedded_${persisted.outcome}`,
    };
  });
}
...
function recordEvidence(options) {
  ...
  return mutateLoop(loopDir, expectedRevision(options), (snapshot, stateRoot) => {
    if (snapshot.provider !== 'verify-agent-output' || snapshot.state !== 'active' || !snapshot.current_iteration)
      throw new LoopValidationError('当前状态不能消费 Evidence');
    validateEvidence(evidence, snapshot);
    if (consumedRunIds(stateRoot).has(evidence.run_id))
      throw new LoopValidationError(`Evidence run_id 已被消费: ${evidence.run_id}`);
    const path = join(loopDir, `evidence-${snapshot.current_iteration.index}.json`);
    if (existsSync(path)) {
      if (canonicalJson(readJson(path)) !== canonicalJson(evidence))
        throw new LoopValidationError('iteration Evidence 文件已存在且内容不同');
    } else writeNewJson(path, evidence);
    const next = applyOutcome(
      { ...snapshot, consumed_verification_run_ids: [...snapshot.consumed_verification_run_ids, evidence.run_id] },
...
function recordVerificationAbort(options) {
  ...
  return mutateLoop(loopDir, expectedRevision(options), (snapshot) => {
    if (
      snapshot.provider !== 'verify-agent-output' ||
      snapshot.state !== 'active' ||
      snapshot.current_iteration?.verification_run_id !== runId
    )
      throw new LoopValidationError('verification abort 与当前 iteration 不匹配');
    return {
      snapshot: {
        ...snapshot,
        state: 'stopped',
        current_iteration: {
          ...snapshot.current_iteration,
          verification_abort: { run_id: runId, code, diagnostics_ref: diagnosticsRef },
        },
        terminal: { kind: 'verification_abort', run_id: runId, code, diagnostics_ref: diagnosticsRef },
      },
      kind: 'verification_abort',
    };
  });
}
...
function nextIteration(options) {
  const loopDir = realpathSync(required(options, 'loop'));
  return mutateLoop(loopDir, expectedRevision(options), (snapshot) => {
    if (snapshot.state !== 'active' || snapshot.current_iteration || snapshot.iterations.at(-1)?.outcome !== 'fail')
      throw new LoopValidationError('当前状态不需要 next');
...
function resume(options) {
  const loopDir = realpathSync(required(options, 'loop'));
  return mutateLoop(loopDir, expectedRevision(options), (snapshot) => {
    if (snapshot.state !== 'stopped' || snapshot.terminal?.kind !== 'verification_abort' || !snapshot.current_iteration)
      throw new LoopValidationError('只允许恢复 verification_abort');
    const runId = required(options, 'verification-run-id');
    if (runId === snapshot.current_iteration.verification_run_id)
      throw new LoopValidationError('resume 必须绑定新的 verification run ID');
    return {
      snapshot: {
        ...snapshot,
        state: 'active',
        terminal: null,
        convergence_report_ref: null,
        current_iteration: { ...snapshot.current_iteration, verification_run_id: runId, verification_abort: null },
        next_action: 'verify',
      },
      kind: 'verification_resumed',
    };
  });
}
...
function stop(options) {
  const loopDir = realpathSync(required(options, 'loop'));
  return mutateLoop(loopDir, expectedRevision(options), (snapshot) => {
    if (!['active', 'waiting_human'].includes(snapshot.state))
      throw new LoopValidationError(`不能停止状态 ${snapshot.state}`);
    return {
      snapshot: {
        ...snapshot,
        state: 'stopped',
        terminal: { kind: 'controller_stop', reason: options.reason ?? 'explicit stop' },
      },
      kind: 'stopped',
    };
  });
...
function humanGate(options) {
  ...
  return mutateLoop(loopDir, expectedRevision(options), (snapshot) => {
    if (snapshot.state !== 'waiting_human' || !snapshot.human_gate.required)
      throw new LoopValidationError('当前没有 pending H gate');
    if (decision === 'approved')
      return {
        snapshot: {
          ...snapshot,
          state: 'completed',
          human_gate: { ...snapshot.human_gate, status: 'approved' },
          terminal: { kind: 'completed', outcome: 'pass' },
        },
    ...
    return {
      snapshot: {
        ...snapshot,
        state: 'stopped',
        human_gate: { ...snapshot.human_gate, status: 'rejected' },
        terminal: { kind: 'human_gate_rejected' },
      },
      kind: 'human_gate_rejected',
    };
  });
}
...
function validateLoop(options) {
  ...
  if (
    ...
    throw new LoopValidationError('冻结 envelope 与 snapshot 不一致');
  for (const iteration of snapshot.iterations) {
    const path = iteration.evidence_digest
      ? join(loopDir, `evidence-${iteration.index}.json`)
      : join(loopDir, `embedded-${iteration.index}.json`);
    const value = readJson(path);
    const field = iteration.evidence_digest ? 'evidence_digest' : 'record_digest';
    if (
      envelopeDigest(value, field) !== value[field] ||
      value[field] !== (iteration.evidence_digest ?? iteration.embedded_record_digest)
    )
      throw new LoopValidationError(`iteration ${iteration.index} 摘要无效`);
    if (!iteration.evidence_digest) {
      for (const check of value.l0_result?.checks ?? []) {
        const logPath = join(loopDir, check.log_ref);
        if (!existsSync(logPath) || sha256(readFileSync(logPath)) !== check.log_digest)
          throw new LoopValidationError(`iteration ${iteration.index} 日志摘要无效: ${check.check_id}`);
      }
    }
  }
  for (const ref of snapshot.reflection_refs ?? []) {
    const value = readAndValidateReflection(
      join(loopDir, ref.ref),
      'run-agent-verify-loop',
      parseJsonStrict,
      envelopeDigest,
    );
    if (value.reflection_digest !== ref.reflection_digest)
      throw new LoopValidationError(`Reflection digest 无效: ${ref.ref}`);
...
function doctor(options) {
  ...
  const substance = substanceWarnings(
    readJson(join(loopDir, 'contract.json')),
    readJson(join(loopDir, 'profile.json')),
  );
  return {
    healthy: !loaded.needsRepair && !lockPresent && !drift && !identityError,
    loop_id: loaded.snapshot.loop_id,
    ...
    state_root_identity_valid: !identityError,
    findings,
    substance_warnings: substance,
    diagnostics: identityError,
    recovery_command: identityError ? `adopt-root --state-root ${stateRoot}` : null,
  };
}
...
export function main(argv = process.argv.slice(2)) {
  ...
  switch (command) {
    case 'capabilities':
      return capabilities();
    case 'adopt-root':
      return adoptStateRoot(options);
    case 'init':
      return initialize(options, flags);
    case 'record-artifact':
      return recordArtifact(options);
    case 'run-embedded-l0':
      return runEmbeddedL0(options);
    case 'record-embedded-review':
      return recordEmbeddedReview(options);
    case 'record-evidence':
      return recordEvidence(options);
    case 'record-verification-abort':
      return recordVerificationAbort(options);
    case 'next':
      return nextIteration(options);
    case 'resume':
      return resume(options);
    case 'stop':
      return stop(options);
    case 'human-gate':
      return humanGate(options);
    case 'record-reflection':
      return recordReflection(options);
    case 'convergence-report':
      return convergenceReport(options);
...
export function runCli(argv = process.argv.slice(2)) {
  try {
    ...
    const code =
      error instanceof LoopOperationalAbort
        ? error.code
        : error instanceof LoopValidationError
          ? 'invalid_input'
          : 'runtime_error';
