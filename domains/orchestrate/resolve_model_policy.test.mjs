...
function args(root, overrides = {}) {
  return {
    ...
    failure_kind: null,
    failure_ref: null,
    available_model: ['sol', 'terra', 'opus', 'luna', 'sonnet'],
    available_effort: ['low', 'medium', 'high', 'xhigh'],
    available_channel: ['worker'],
    explain: false,
    ...overrides,
  };
}
      ...
      () =>
        resolve(
          args(root, {
            ...
            action: 'promote_tier',
            tier: 'frontier',
            failure_kind: 'environment_fault',
            failure_ref: FAILURE_REF,
            selection_reason: '错误地用模型处理环境故障',
          }),
        ),
...
test('CLI rejects missing option values before applying defaults', () => {
  assert.throws(
    () => parseCli(['--host', 'test-host', '--tier', 'primary', '--selection-reason', 'reason', '--config-dir']),
    /--config-dir 缺少值/,
  );
  assert.throws(
    () =>
