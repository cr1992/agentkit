...
function status(options) { const dir = ledgerDir(options); const loaded = load(dir); const nodes = Object.values(loaded.snapshot.nodes); const totalTokens = nodes.reduce((acc, n) => acc + (typeof n.tokens === 'number' ? n.tokens : (n.tokens?.total_tokens ?? 0)), 0); const tokensByRole = {}; for (const n of nodes) { const t = typeof n.tokens === 'number' ? n.tokens : (n.tokens?.total_tokens ?? 0); tokensByRole[n.role] = (tokensByRole[n.role] || 0) + t; }
  // 覆盖规则只在契约声明了 verify-agent-output provider 时生效：声明了就必须真的验到最终产物。
  const declared = readJson(join(dir, 'contract.json')).extensions?.verification?.provider === 'verify-agent-output';
  const covered = declared ? integrationCoverage(loaded.snapshot.nodes, loaded.snapshot.edges) : new Set();
  ...
  // 空 ledger（没有任何 required 节点）不算完成：`[].every(...)` 的空真会让覆盖规则和后续 close 门禁一起失效。
  return { ...loaded.snapshot, recovery_needed: loaded.repair, summary: { pending: nodes.filter((n) => n.state === 'pending').length, active: nodes.filter((n) => ['running', 'blocked', 'awaiting_verification'].includes(n.state)).length, terminal: nodes.filter((n) => TERMINAL_NODE_STATES.has(n.state)).length, completion_ready: required.length > 0 && required.every((n) => n.state === 'passed') && !uncovered.length, uncovered_implementation_nodes: uncovered, non_required_implementation_nodes: nodes.filter((n) => !n.required && implementationNode(n)).map((n) => n.node_id), nodes_without_independent_evidence: declared ? [] : nodes.filter((n) => implementationNode(n) && n.verification_assurance !== 'independent_evidence').map((n) => n.node_id), verification_assurance: { none: nodes.filter((n) => !n.verification_assurance || n.verification_assurance === 'none').length, worker_self_check: nodes.filter((n) => n.verification_assurance === 'worker_self_check').length, controller_recheck: nodes.filter((n) => n.verification_assurance === 'controller_recheck').length, independent_evidence: nodes.filter((n) => n.verification_assurance === 'independent_evidence').length, not_applicable: nodes.filter((n) => n.verification_assurance === 'not_applicable').length }, token_accounting: { total_tokens: totalTokens, by_role: tokensByRole } } }; }
