...
function assertOpen(snapshot) { const life = snapshot.lifecycle; if (!life) return; throw new LedgerError(`ledger 已处于终态：lifecycle.state=${life.state}，closed_at=${life.closed_at}；任务图已冻结，之后只剩 record-reflection / propose-improvement / rebuild / doctor / status / inspect / batch-status`); }
...
const CLI_NOTES = [
  ...
  '终态之后任务图冻结，只剩 record-reflection / propose-improvement / rebuild / doctor / status / inspect / batch-status。',
];
// 终态之后仍然放行的命令：架构 §15.2 的高优先级反思触发按定义发生在完成之后；rebuild 是崩溃修复；其余是只读回看。
const POST_TERMINAL_ALLOWED = new Set(['capabilities', 'init', 'record-reflection', 'propose-improvement', 'rebuild', 'doctor', 'status', 'inspect', 'batch-status']);
...
// batch-fuse 今天只读快照，但它属于批级任务图的收口动作而不是回看：终态之后一并冻结，与 batch-status 分开。
function batchFuse(options) { const dir = ledgerDir(options); assertOpen(load(dir).snapshot); const batch = batchStatus(options); return { batch_id: batch.batch_id, state: batch.state, fuse: batch.fuse ?? evaluateBatchFuse(batch) }; }
