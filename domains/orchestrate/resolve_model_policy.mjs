...
export const ADJUSTMENT_ACTIONS = new Set([
  'retry_same',
  'raise_effort',
  'switch_model',
  'promote_tier',
...
function readJson(path, label = path) {
  try {
    ...
    throw new PolicyError(`cannot read ${label}: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
function loadHostConfig(path, expectedHost) {
  ...
  if (config.schema_version !== CONFIG_SCHEMA_VERSION)
    throw new PolicyError(`${path} must declare schema_version ${CONFIG_SCHEMA_VERSION}`);
  ...
  if (
    Object.keys(tiers).length !== tierOrder.length ||
    tierOrder.some((name) => !Object.hasOwn(tiers, name)) ||
    Object.keys(tiers).some((name) => !tierOrder.includes(name))
  ) {
  ...
  for (const [name, raw] of Object.entries(tiers)) {
    ...
    for (const key of EFFORT_KEYS) {
      if (typeof effort[key] !== 'string' || !effortRank.has(effort[key]))
        throw new PolicyError(`${path}.tiers.${name}.effort.${key} must appear in effort_order`);
    }
    if (
      effortRank.get(effort.min) > effortRank.get(effort.default) ||
      effortRank.get(effort.default) > effortRank.get(effort.max)
    )
      throw new PolicyError(`${path}.tiers.${name}.effort must satisfy min <= default <= max`);
    if (typeof tier.channel !== 'string' || !tier.channel.trim())
      throw new PolicyError(`${path}.tiers.${name}.channel must be a non-empty string`);
  ...
  if (!Number.isSafeInteger(dynamic.max_attempts) || dynamic.max_attempts < 1)
    throw new PolicyError(`${path}.dynamic_adjustment.max_attempts must be a positive safe integer`);
  ...
  if (unknownActions.length)
    throw new PolicyError(
      `${path}.dynamic_adjustment.allowed_actions has invalid actions: ${unknownActions.join(', ')}`,
    );
  return config;
}
...
function chooseModel(tier, explicitModel, availableModels, userExplicit) {
  if (explicitModel) {
    if (!userExplicit && !tier.models.includes(explicitModel))
      throw new PolicyError(`model ${JSON.stringify(explicitModel)} is outside the selected local tier`);
    if (availableModels.length && !availableModels.includes(explicitModel))
      throw new PolicyError(`model ${JSON.stringify(explicitModel)} is not exposed by the current host schema`);
    return explicitModel;
  }
  if (!availableModels.length)
    throw new PolicyError('model discovery is unavailable; persisted tier candidates cannot be validated');
...
function validateEffort(tier, effortOrder, effort, availableEfforts) {
  ...
  if (
    effortRank.get(effort) < effortRank.get(tier.effort.min) ||
    effortRank.get(effort) > effortRank.get(tier.effort.max)
  )
    throw new PolicyError(
      `effort ${JSON.stringify(effort)} is outside tier range ${JSON.stringify(tier.effort.min)}..${JSON.stringify(tier.effort.max)}`,
    );
  if (availableEfforts.length && !availableEfforts.includes(effort))
    throw new PolicyError(`effort ${JSON.stringify(effort)} is not exposed by the current host schema`);
}

function validateAdjustment(args, config, selection, previous) {
  if (args.attempt === 1) {
    if (previous || args.action || args.failure_kind || args.failure_ref)
      throw new PolicyError('initial dispatch cannot declare retry inputs');
  ...
  if (args.attempt > config.dynamic_adjustment.max_attempts)
    throw new PolicyError(
      `attempt ${args.attempt} exceeds local max_attempts ${config.dynamic_adjustment.max_attempts}`,
    );
  ...
  if (
    previous.schema_version !== RESULT_SCHEMA_VERSION ||
    previous.record_type !== 'model-policy-resolution' ||
    previous.host !== args.host ||
    !previousPatch ||
    typeof previousPatch !== 'object' ||
    Array.isArray(previousPatch)
  )
    throw new PolicyError('previous resolution schema/host mismatch');
  if (previousPatch.attempt !== args.attempt - 1)
    throw new PolicyError('previous resolution attempt is not immediately prior');
  if (!ADJUSTMENT_ACTIONS.has(args.action)) throw new PolicyError('retry dispatch requires a valid --action');
  if (!config.dynamic_adjustment.allowed_actions.includes(args.action))
    throw new PolicyError(`adjustment action ${JSON.stringify(args.action)} is outside local config`);
  if (!FAILURE_KINDS.has(args.failure_kind)) throw new PolicyError('retry dispatch requires a valid --failure-kind');
  if (!REROUTABLE_FAILURES.has(args.failure_kind))
    throw new PolicyError(
      `failure kind ${JSON.stringify(args.failure_kind)} must stop/re-contract instead of model rerouting`,
    );
  if (!DIGEST_PATTERN.test(args.failure_ref || ''))
    throw new PolicyError('retry dispatch requires a sha256 --failure-ref');
  ...
  if (args.action === 'retry_same' && !(sameTier && sameModel && sameEffort))
    throw new PolicyError('retry_same must preserve tier, model and effort');
  if (
    args.action === 'raise_effort' &&
    !(sameTier && sameModel && effortRank.get(selection.effort) > effortRank.get(previousPatch.reasoning_effort))
  )
    throw new PolicyError('raise_effort must keep tier/model and increase effort');
  if (args.action === 'switch_model' && sameModel) throw new PolicyError('switch_model must change the model');
  if (
    args.action === 'promote_tier' &&
    rankOf(config.tier_order, selection.tier, 'tier') <= rankOf(config.tier_order, previousPatch.tier, 'previous tier')
  )
    throw new PolicyError('promote_tier must move to a higher configured tier');
  if (args.action === 'fresh_context' && !(sameTier && sameModel && sameEffort))
    throw new PolicyError('fresh_context changes worker context, not model parameters');
  return {
    action: args.action,
    failure_kind: args.failure_kind,
    failure_ref: args.failure_ref,
    previous_attempt_id: previousPatch.attempt_id,
  };
}
...
export function resolve(args) {
  ...
  if (userExplicit && (!args.model || !args.effort))
    throw new PolicyError('--user-explicit requires explicit --model and --effort values');
  ...
  if (availableChannels.length && !availableChannels.includes(tier.channel))
    throw new PolicyError(`channel ${JSON.stringify(tier.channel)} is not exposed by the current host schema`);
  if ((!availableModels.length || !availableEfforts.length) && !userExplicit)
    throw new PolicyError('persisted local config requires live model and effort discovery');

  const previous = args.previous_dispatch
    ? assertObject(readJson(resolveUserPath(args.previous_dispatch), args.previous_dispatch), args.previous_dispatch)
    : null;
...
export function parseCli(argv = process.argv.slice(2)) {
  ...
  if (!options.selection_reason || /[\r\n]/u.test(options.selection_reason))
    throw new PolicyError('缺少合法 --selection-reason');
  if (!Number.isSafeInteger(options.attempt) || options.attempt < 1)
    throw new PolicyError('--attempt 必须为正安全整数');
  return options;
}
