...
test('同一 Artifact 默认只允许一次 primary review，并拒绝重复 lens', () => {
  ...
  assert.equal(
    evaluateReviewBudget(POLICY, [review({ outcome: 'pass' })], request({ lens: 'runtime_safety' })).code,
    'PRIMARY_REVIEW_LIMIT',
  );
...
test('第二 reviewer 必须是有证据触发的不同 lens escalation，第三个被预算拒绝', () => {
  ...
  const escalation = request({
    lens: 'verification_definition',
    kind: 'escalation',
    escalation_trigger: 'undecidable',
  });
  assert.equal(evaluateReviewBudget(POLICY, [first], escalation).code, 'ALLOWED_ESCALATION');
  const history = [
    first,
    review({ review_id: 'review-2', lens: 'verification_definition', kind: 'escalation', outcome: 'pass' }),
  ];
  assert.equal(
    evaluateReviewBudget(
      POLICY,
      history,
      request({ lens: 'runtime_safety', kind: 'escalation', escalation_trigger: 'protocol_ambiguity' }),
    ).code,
    'ESCALATION_REVIEW_LIMIT',
  );
  assert.equal(
    evaluateReviewBudget(POLICY, [review({ outcome: 'fail' })], escalation).code,
    'UNDECIDABLE_EVIDENCE_MISSING',
  );
});
...
test('smoke、输入 token 与 safety 都是 reviewer 派发前的硬门禁', () => {
  ...
  assert.equal(
    evaluateReviewBudget(POLICY, [], request({ estimated_input_tokens: 12_001 })).code,
    'REVIEW_INPUT_BUDGET_EXCEEDED',
  );
  assert.equal(
    evaluateReviewBudget(
      POLICY,
      [review({ outcome: 'blocked_safety' })],
      request({ lens: 'safety_confirmation', kind: 'escalation', escalation_trigger: 'protocol_ambiguity' }),
    ).code,
    'SAFETY_STOP',
  );
...
test('history 只接受数组，报错写明空历史与已有 review 两种最小形状', () => {
  ...
  for (const wrong of [{ schema_version: 1, reviews: [] }, null, 'none', 0]) {
    ...
    assert.throws(
      () => evaluateReviewBudget(POLICY, wrong, request()),
      /没有历史 review 传 \[\]/u,
      JSON.stringify(wrong),
    );
    assert.throws(
      () => evaluateReviewBudget(POLICY, wrong, request()),
      /review_id, artifact_digest, lens, kind, outcome/u,
      JSON.stringify(wrong),
    );
  }
  assert.equal(evaluateReviewBudget(POLICY, [], request()).code, 'ALLOWED_PRIMARY');
  ...
  try {
    ...
    const denied = spawnSync(
      ...
      [
        ...
        write('history.json', []),
        '--request',
        write('request.json', request({ smoke_passed: false })),
      ],
      { encoding: 'utf8' },
    );
