...
test('project 从父合同 verbatim 切出条目子集，并写入可校验血缘', () => {
  ...
  try {
    f.value.acceptance = [
      { contract_item_id: 'a', requirement: 'A 通过' },
      { contract_item_id: 'b', requirement: 'B 通过' },
      { contract_item_id: 'c', requirement: 'C 通过' },
    ];
    ...
    for (const field of ['objective', 'scope', 'permissions', 'environment', 'skill_set', 'stop_conditions'])
      assert.deepEqual(projected[field], parent[field]);

    // parent 的额外顶层字段原样继承，不被静默丢弃
    const withExtra = { ...parent, owner_team: 'moii-app', custom_budget: { max_rounds: 3 } };
    const extraPath = join(f.root, 'with-extra.json');
    writeFileSync(extraPath, JSON.stringify(withExtra));
    const signedExtra = main(['normalize', '--input', extraPath]);
    writeFileSync(extraPath, JSON.stringify(signedExtra));
    ...
    const unsignedPath = join(f.root, 'unsigned.json');
    ...
    const driftedPath = join(f.root, 'drifted.json');
