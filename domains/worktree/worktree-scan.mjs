...
export function runCli(argv = process.argv.slice(2)) {
  ...
  function recentCommitFiles() {
    let out;
    try {
      out = git(['log', '--all', '--since=6 hours ago', '--name-only', '-z', '--pretty=format:'], { cwd: ROOT });
    } catch {
      return [];
    }
    const files = [];
  ...
  function findTasksFiles() {
    ...
    return expandRepositoryGlob(PROFILE.context.primary_worktree ?? ROOT, String(KIRO_TASK_SOURCE.glob));
  }
  ...
  function pathMatch(target, inflight, allowRelative) {
    ...
    if (
      (target.length > inflight.length && relativeMatch(target, inflight)) ||
      (inflight.length > target.length && relativeMatch(inflight, target))
    )
      return { kind: 'suffix-relative', confidence: 'low' };
    return null;
  }
  ...
  for (const t of targets) {
    for (const f of sortedFiles) {
      ...
      const match = pathMatch(
        t,
        f,
        [...sourceSet].some((source) => source === 'task:[-]'),
      );
      if (match) {
        collisions.push({
          target: t,
  ...
  console.log(`${PREFIX} 结论: COLLIDE — 目标文件有别人在动，建议开 worktree 隔离（DESIGN §2.1：会撞=开 worktree）。`);
...
export function isCliEntry(argv1 = process.argv[1], selfUrl = import.meta.url) {
  ...
  try {
    ...
    return path.resolve(argv1) === path.resolve(fileURLToPath(selfUrl));
  }
}
