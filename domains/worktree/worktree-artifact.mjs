...
export function createCommands(deps) {
  ...
  function verifyArtifactEnvelope(artifact, loaded, records = loadRecords(loaded.context.common_dir)) {
    if (!artifact || artifact.schema_version !== 1 || artifact.provider !== 'manage-worktrees')
      throw new Error('Artifact schema/provider 无效');
    ...
    if (!Number.isInteger(artifact.ownership_epoch) || artifact.ownership_epoch < 1)
      throw new Error('Artifact ownership_epoch 缺失或无效');
    const identity = readRepositoryIdentity(loaded.context);
    if (!identity || artifact.repository_id !== identity.repository_id)
      throw new Error('Artifact repository_id 不匹配');
    ...
    for (const field of ['base_sha', 'artifact_sha']) {
      if (!new RegExp(`^[0-9a-f]{${length}}$`, 'u').test(String(artifact[field] ?? '')))
        throw new Error(`${field} 不是完整 object id`);
      if (!gitTry(['cat-file', '-e', `${artifact[field]}^{commit}`], loaded.context.current_worktree).ok)
        throw new Error(`${field} 不是可达 commit`);
    }
    if (
      !gitTry(
        ['merge-base', '--is-ancestor', artifact.base_sha, artifact.artifact_sha],
        loaded.context.current_worktree,
      ).ok
    )
      throw new Error('Artifact base 不是 artifact ancestor');
    ...
    if (artifact.ownership_epoch !== (record.ownership_epochs?.length ?? 0))
      throw new Error('Artifact ownership_epoch 已 stale');
