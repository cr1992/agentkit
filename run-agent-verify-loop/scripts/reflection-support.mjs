...
function eventDigest(runDir, id, parseJsonStrict, canonicalJson) {
  const match = /^events\.ndjson#revision=(\d+)$/u.exec(id);
  if (!match) return null;
  const lines = readFileSync(join(runDir, 'events.ndjson'), 'utf8').split('\n').filter(Boolean);
...
export function verifyEvidenceRefs(runDir, refs, parseJsonStrict, canonicalJson) {
  ...
  return refs.map((ref) => {
    ...
    const event = ref.type === 'event' ? eventDigest(runDir, ref.id, parseJsonStrict, canonicalJson) : null;
    const actual = event ?? (() => {
      const relative = safeRelative(ref.id);
...
export function buildReflection({ input, runDir, scope, skill, parseJsonStrict, canonicalJson, envelopeDigest }) {
  ...
  const evidenceRefs = verifyEvidenceRefs(runDir, input.evidence_refs ?? [], parseJsonStrict, canonicalJson);
