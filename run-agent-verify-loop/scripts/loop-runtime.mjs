...
function validUnicode(value) {
  for (let index = 0; index < value.length; index += 1) {
    ...
    if (code >= 0xd800 && code <= 0xdbff) {
      ...
      if (!(next >= 0xdc00 && next <= 0xdfff)) throw new LoopValidationError('字符串含未配对 high surrogate');
      index += 1;
    } else if (code >= 0xdc00 && code <= 0xdfff) throw new LoopValidationError('字符串含未配对 low surrogate');
  }
}
...
export function canonicalJson(value) {
  ...
  if (typeof value === 'number') {
    if (!Number.isFinite(value)) throw new LoopValidationError('canonical JSON 不接受非有限 number');
  ...
  throw new LoopValidationError(`canonical JSON 不支持 ${typeof value}`);
}
...
function writeNewJson(path, value) { writeFileSync(path, `${JSON.stringify(value, null, 2)}\n`, { flag: 'wx', mode: 0o600 }); }
...
export function skillContentDigest(root = SKILL_ROOT) { return sha256(Buffer.from(canonicalJson(manifestFiles(realpathSync(root))), 'utf8')); }
...
function doctor(options) {
  ...
  const drift = skillContentDigest() !== loaded.snapshot.skill_provenance.content_digest;
  const lockPresent = existsSync(join(stateRoot, '.loop-runtime.lock'));
  const findings = identityError ? ['state_root_identity_invalid'] : [];
  return { healthy: !loaded.needsRepair && !lockPresent && !drift && !identityError, loop_id: loaded.snapshot.loop_id, revision: loaded.snapshot.revision, snapshot_matches_journal: !loaded.needsRepair, lock_present: lockPresent, skill_drift: drift, state_root_identity_valid: !identityError, findings, diagnostics: identityError, recovery_command: identityError ? `adopt-root --state-root ${stateRoot}` : null };
}
...
if (isCliEntry(import.meta.url, process.argv[1])) {
  try { process.stdout.write(`${JSON.stringify(main(), null, 2)}\n`); }
    ...
    process.exit(error instanceof LoopValidationError ? 2 : 3);
  }
}
