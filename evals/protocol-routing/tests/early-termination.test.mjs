...
const useLine = (id, name, input) => `${JSON.stringify({ type: 'assistant', message: { model: 'm', content: [{ type: 'tool_use', id, name, input }] } })}\n`;

test('哪些用例允许提前终止：正向且只看第一个观测量的那几条', () => {
  assert.deepEqual(CASES.filter(canTerminateEarly).map((item) => item.id), [1, 2, 3, 4, 6]);
...
test('增量扫描：按行吃 stream-json，半行留到下一个 chunk，只认 tool_use', () => {
  ...
  assert.deepEqual(scanner.uses.map((use) => use.id), ['t1']);

  // 一条 assistant 事件里两个并行 tool_use，按出现顺序都收下。
  const two = JSON.stringify({ type: 'assistant', message: { content: [
    { type: 'tool_use', id: 't2', name: 'Read', input: {} },
    { type: 'tool_use', id: 't3', name: 'Bash', input: { command: 'ls' } },
  ] } });
  assert.equal(scanner.push(`${two}\n`), 2);
  assert.deepEqual(scanner.uses.map((use) => use.id), ['t1', 't2', 't3']);
...
function firstSatisfiedAt(evalCase, steps) {
  ...
  for (const step of steps) {
    scanner.push(useLine(step.id, step.tool_name ?? 'Bash', step.command === undefined ? {} : { command: step.command }));
    ...
    const events = scanner.uses.map((use, index) => {
      ...
      return { seq: index + 1, tool_name: use.tool_name, tool_input: use.tool_input, repo: probe.repo, ledger: probe.ledger };
    });
...
test('正向断言一成立就能收手，而且收手前后的判定是同一个结论', () => {
  ...
  assert.equal(firstSatisfiedAt(caseById(3), [
  ...
  assert.equal(at, 2);
  const events = (steps) => steps.map((step, index) => ({ seq: index + 1, tool_name: step.tool_name ?? 'Bash', tool_input: step.command === undefined ? {} : { command: step.command }, repo: step.repo ?? { status: '', head: 'a'.repeat(40) }, ledger: null }));
  const truncated = caseById(4).assert(classify({ initial_repo: { status: '', head: 'a'.repeat(40) }, initial_ledger: null, events: events(full.slice(0, at)) }), {});
  const complete = caseById(4).assert(classify({ initial_repo: { status: '', head: 'a'.repeat(40) }, initial_ledger: null, events: events(full) }), {});
...
test('被 harness 掐掉的会话是有效数据点，不当崩溃重试', () => {
  ...
  const early = classifyRunValidity({
    events: [{}, {}],
    end: { exit_code: null, host_result: null, early_terminated: { at_seq: 2, reason: '路由到 agentkit verify prepare' } },
  });
  assert.equal(early.valid, true);
  assert.match(early.reason, /第 2 个事件成立/u);

  // 非零退出也一样。
  assert.equal(classifyRunValidity({ events: [{}], end: { exit_code: 143, host_result: null, early_terminated: { at_seq: 1, reason: 'x' } } }).valid, true);

  // 没标 early_terminated 的照旧走故障判据，一个字都没松。
  assert.equal(classifyRunValidity({ events: [], end: { exit_code: 1, host_result: null } }).valid, false);
  assert.equal(classifyRunValidity({ events: [{}], end: { exit_code: 0, host_result: { is_error: true, subtype: 'success' } } }).valid, false);
});
