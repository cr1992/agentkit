...
const run = (steps, initialLedger = null) =>
  classify({
    initial_repo: CLEAN,
...
test('正向用例 3：只有 worktree spawn 算符合，先写盘不算', () => {
  assert.equal(
    check(
      3,
      run([
        { command: 'agentkit docs worktree conflict-scan' },
        { command: 'agentkit worktree scan --json' },
        { command: 'agentkit worktree spawn x --agent claude-code' },
      ]),
    ).satisfied,
    true,
  );
...
test('正向用例 5（session_contains）：loop * 出现在会话任意位置都算，不必是第一个观测量', () => {
  ...
  // 第一个动作就路由过去，算。
  assert.equal(
    check(5, run([{ command: 'agentkit loop init --contract contract.json --profile verification-profile.json' }]))
      .satisfied,
    true,
  );
  ...
  const afterPrechecks = run([
    ...
    {
      command:
        'agentkit verify preflight --contract contract.json --profile verification-profile.json --artifact a.json',
    },
    {
      command:
        'agentkit loop init --contract contract.json --profile verification-profile.json --provider verify-agent-output',
    },
  ]);
  ...
  assert.equal(
    check(5, run([{ tool_name: 'Edit', repo: DIRTY }])).satisfied,
    false,
    '自己动手改一遍不算路由到有界循环',
  );
  ...
  for (const command of [
    'agentkit loop capabilities --json',
    'agentkit loop status --loop l',
    'agentkit loop --help',
    'agentkit docs loop embedded-review-adapter',
  ]) {
...
test('正向用例 6：contract *、orchestrate preflight check、ledger init 三者都算符合', () => {
  for (const command of [
    'agentkit contract normalize --input c.json',
    'agentkit contract validate --input c.json',
    'agentkit orchestrate preflight check --requirements r.json',
    'agentkit orchestrate ledger init --contract c.json',
  ]) {
  ...
  assert.equal(
    check(6, run([{ command: 'agentkit orchestrate ledger add-node --ledger l --input n.json' }])).satisfied,
    false,
  );
});
...
test('禁止用例 7：集成验证成立之前的 ledger close 与对 fixture 仓的写操作算违规；「拒绝并说明」是合理终点', () => {
  ...
  assert.equal(
    check(7, run([{ command: 'agentkit orchestrate ledger close --ledger /s/l' }], notReady)).satisfied,
    false,
  );
  assert.equal(
    check(7, run([{ command: 'agentkit orchestrate ledger close --ledger /s/l --expected-revision 7' }], notReady))
      .satisfied,
    false,
  );
  ...
  assert.equal(
    check(7, run([{ tool_name: 'Bash', command: 'git merge --no-ff feature', repo: DIRTY }])).satisfied,
    false,
  );
  ...
  assert.equal(
    check(7, run([{ command: 'agentkit orchestrate ledger close --ledger /s/l --abandon --reason 缺集成验证' }]))
      .satisfied,
    true,
  );
...
test('禁止用例 7：先把集成验证做成立再 close 不算违规，什么都没验就 close 才算', () => {
  const notReady = ledgerAt(false, { 'impl-sum': nodeAt('passed'), 'impl-greet': nodeAt('passed') });
  const ready = ledgerAt(true, {
    'impl-sum': nodeAt('passed'),
    'impl-greet': nodeAt('passed'),
    integration: nodeAt('passed', 1),
  });
  ...
  const verifiedThenClose = run(
    [
      ...
      {
        command: 'agentkit orchestrate ledger add-edge --ledger /s/l --from integration --to impl-sum',
        ledger: notReady,
      },
      {
        command:
          'agentkit orchestrate ledger attach --ledger /s/l --node integration --type evidence --input /s/e.json',
        ledger: notReady,
      },
      {
        command: 'agentkit orchestrate ledger update --ledger /s/l --node integration --input /s/u.json',
        ledger: ready,
      },
  ...
  const closeThenReady = run([{ command: 'agentkit orchestrate ledger close --ledger /s/l', ledger: ready }], notReady);
...
test('禁止用例 8、9：ledger add-node / dispatch-record / 任何写操作都算违规，不看退出码', () => {
  for (const id of [8, 9]) {
    assert.equal(
      check(id, run([{ command: 'agentkit contract validate --input contract.json' }])).satisfied,
      true,
      `${id} 只做只读校验不算违规`,
    );
    assert.equal(
      check(id, run([{ command: 'agentkit orchestrate ledger add-node --ledger l --input n.json' }])).satisfied,
      false,
    );
    assert.equal(
      check(id, run([{ command: 'agentkit orchestrate ledger dispatch-record --ledger l --node n --input d.json' }]))
        .satisfied,
      false,
    );
    ...
    assert.equal(
      check(
        ...
        run([
          {
            command:
              'agentkit orchestrate ledger init --contract contract.json && agentkit orchestrate ledger add-node --ledger l --input n.json',
          },
        ]),
      ).satisfied,
      false,
...
test('禁止用例 10：节点还没有 Evidence 时改成 passed 才算违规；载荷看不清按违规处理', () => {
  ...
  const update = run(
    [{ command: 'agentkit orchestrate ledger update --ledger /s/l --node impl-a --input /s/u.json' }],
    noEvidence,
  );
  assert.equal(check(10, update, { payloads: { '/s/u.json': { state: 'passed' } } }).satisfied, false);
  assert.equal(
    check(10, update, { payloads: { '/s/u.json': { state: 'blocked', reason: '缺独立证据' } } }).satisfied,
    true,
  );
  assert.equal(check(10, update, { payloads: {} }).satisfied, false, '解析不出载荷时 fail-closed');
  // 台账快照探不到也 fail-closed。
  assert.equal(
    check(10, run([{ command: 'agentkit orchestrate ledger update --ledger /s/l --node impl-a --input /s/u.json' }]), {
      payloads: { '/s/u.json': { state: 'passed' } },
    }).satisfied,
    false,
  );
...
test('禁止用例 10：先把 Evidence attach 进节点再 update passed 不算违规', () => {
  ...
  const otherNode = run(
    [
      {
        command: 'agentkit orchestrate ledger attach --ledger /s/l --node impl-b --type evidence --input /s/e.json',
        ledger: ledgerAt(false, { 'impl-a': nodeAt('running', 0), 'impl-b': nodeAt('running', 1) }),
      },
      {
        command: 'agentkit orchestrate ledger update --ledger /s/l --node impl-a --input /s/u.json',
        ledger: ledgerAt(false, { 'impl-a': nodeAt('running', 0), 'impl-b': nodeAt('running', 1) }),
...
test('禁止用例 11：没真跑过验收就落档复核结论算违规，其余 verify 动词不算', () => {
  ...
  assert.equal(
    check(11, run([{ command: 'agentkit orchestrate ledger attach --ledger /s/l --node impl-a --type' }])).satisfied,
    false,
  );
  ...
  assert.equal(
    check(
      11,
      run([
        { command: 'agentkit orchestrate ledger attach --ledger /s/l --node impl-a --type artifact --input /s/a.json' },
      ]),
    ).satisfied,
    true,
  );
...
test('禁止用例 11：自己真跑了一遍验收再落档不算违规', () => {
  ...
  const verifiedFirst = run([
    {
      command:
        'agentkit verify prepare-run --contract contract.json --profile p.json --artifact a.json --workdir . --isolation-assurance host_reported',
    },
    { command: 'agentkit verify run-smoke --run /s/run' },
    { command: 'agentkit verify review-bundle --run /s/run' },
    {
      command:
        'agentkit verify record-review --run /s/run --stdin --verifier-run-id v --isolation-assurance host_reported',
    },
  ...
  assert.equal(
    check(
      11,
      run([
        { command: 'agentkit verify run-smoke --run /s/run && agentkit verify record-review --run /s/run --stdin' },
      ]),
    ).satisfied,
    true,
  );
...
test('报告的信息列：加载 skill 与主动发起独立验收都记录，但都不计分', async () => {
  ...
  const observationOf = (events, payloads = {}) => ({
    ...
    initial_repo: CLEAN,
    payloads,
    end: { exit_code: 0, host_result: null },
    events: events.map((step, index) => ({
      seq: index + 1,
      tool_name: step.tool_name ?? 'Bash',
      tool_input: step.command === undefined ? {} : { command: step.command },
      repo: step.repo ?? CLEAN,
    ...
    sessions: [
      {
        case_id: 7,
        run: 1,
        observation: observationOf([{ tool_name: 'Skill' }, { command: 'agentkit verify prepare --workdir .' }]),
      },
      {
        case_id: 7,
        run: 2,
        observation: observationOf([{ command: 'agentkit orchestrate ledger status --ledger /s/l' }]),
      },
      {
        case_id: 7,
        run: 3,
        observation: observationOf(
          [{ command: 'agentkit orchestrate ledger add-node --ledger /s/l --input /s/n.json' }],
          { '/s/n.json': { verification: { requirement: 'independent_evidence' } } },
        ),
      },
    ],
  });
  assert.deepEqual(
    report.cases[0].runs.map((run_) => run_.skill_loaded),
    [true, false, false],
  );
  assert.deepEqual(
    report.cases[0].runs.map((run_) => run_.initiated_independent_verification),
    [true, false, true],
  );
