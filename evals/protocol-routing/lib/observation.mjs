 ...
 * @typedef {{ seq: number, tool_name: string, tool_input?: any, repo: RepoSummary }} ToolEvent
 * @typedef {{
...
export function parseObservation(text) {
  /** @type {Observation} */
  const out = { meta: {}, initial_repo: { status: '', head: '' }, events: [], payloads: {}, end: null };
  let seen = false;
  for (const line of text.split('\n')) {
    ...
    if (record.type === 'session') {
      const { type, initial_repo: initial, ...meta } = record;
      out.meta = meta;
      out.initial_repo = initial ?? out.initial_repo;
      seen = true;
    } else if (record.type === 'tool') {
      out.events.push({ seq: record.seq ?? out.events.length + 1, tool_name: record.tool_name, tool_input: record.tool_input, repo: record.repo });
...
export function serializeObservation(observation) {
  const lines = [JSON.stringify({ type: 'session', ...observation.meta, initial_repo: observation.initial_repo })];
