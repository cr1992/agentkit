...
const REFLECTION_INPUT_KEYS = new Set([
  'contract_digest',
  'trigger',
  'classification',
  'observation',
...
const PROPOSAL_INPUT_KEYS = new Set([
  'problem_type',
  'proposed_change',
  'affected_scope',
  'counterexamples',
...
export function safeRelative(id) {
  if (typeof id !== 'string' || !id || isAbsolute(id) || id.includes('\\'))
    throw new Error('evidence ref id 必须是相对路径');
...
export function createReflectionKit({ strict = false } = {}) {
  ...
  const eventDigest = (stateDir, id, parseJsonStrict, canonicalJson) => {
    ...
    const event = lines
      ...
      .find((candidate) => candidate.revision === Number(match[1]));
  ...
  function verifyEvidenceRefs(stateDir, refs, parseJsonStrict, canonicalJson) {
    ...
    return refs.map((ref) => {
      if (!ref || !REF_TYPES.has(ref.type) || typeof ref.id !== 'string' || !DIGEST.test(String(ref.digest ?? '')))
        throw new Error('evidence ref 结构无效');
      ...
      const actual =
        ...
        (() => {
          ...
          if (fromRoot === '..' || fromRoot.startsWith(`..${sep}`) || isAbsolute(fromRoot))
            throw new Error(`evidence ref 越出 state 目录: ${ref.id}`);
  ...
  function buildReflection({ input, stateDir, runDir, scope, skill, parseJsonStrict, canonicalJson, envelopeDigest }) {
    ...
    for (const forbidden of ['chain_of_thought', 'reasoning', 'scratchpad'])
      if (Object.hasOwn(input, forbidden)) throw new Error(`Reflection 禁止字段: ${forbidden}`);
    if (
      !TRIGGERS.has(input.trigger) ||
      !CLASSIFICATIONS.has(input.classification) ||
      !IMPACTS.has(input.impact) ||
      !CONFIDENCE.has(input.confidence) ||
      !DISPOSITIONS.has(input.recommended_disposition)
    )
      throw new Error('Reflection 枚举字段无效');
    const evidenceRefs = verifyEvidenceRefs(root, input.evidence_refs ?? [], parseJsonStrict, canonicalJson);
    if (evidenceRefs.length === 0 && input.confidence !== 'low')
      throw new Error('无稳定证据的 Reflection 只能是 low confidence');
    const record = {
      schema_version: 1,
  ...
  function readAndValidateReflection(path, expectedSkill, parseJsonStrict, envelopeDigest) {
    ...
    if (
      reflection?.schema_version !== 1 ||
      reflection.affected_skill?.name !== expectedSkill ||
      !DIGEST.test(String(reflection.reflection_digest ?? '')) ||
      envelopeDigest(reflection, 'reflection_digest') !== reflection.reflection_digest
    )
      throw new Error('Reflection 文件无效或目标 Skill 不匹配');
    return reflection;
  }
  ...
  function buildProposal({ input, reflections, skill, envelopeDigest }) {
    ...
    if (input.lifecycle !== undefined && input.lifecycle !== 'proposed')
      throw new Error('执行 Skill 只能创建 proposed proposal');
    ...
    if (
      ...
      (typeof validationPlan !== 'object' ||
        ...
        Object.keys(validationPlan).some((key) => !VALIDATION_PLAN_KEYS.has(key)))
    )
      throw new Error('proposal validation_plan 含未知字段');
    if (
      !validationPlan ||
      !Array.isArray(validationPlan.replay_cases) ||
      !Array.isArray(validationPlan.regression_suites) ||
      validationPlan.independent_review !== 'required'
    )
      throw new Error('proposal validation_plan 无效');
    const proposal = {
      schema_version: 1,
      proposal_id: randomUUID(),
      target_skill: { name: skill.name, based_on_version: skill.version, based_on_digest: skill.content_digest },
      source_reflections: reflections.map((item) => ({
        reflection_id: item.reflection_id,
        reflection_digest: item.reflection_digest,
      })),
